...
Output, per corpus, in <corpus>/corpus/prelim/ :
    baseline_ladder.txt        the table plus a per-author report
    confusion_<model>.png      confusion matrix for the best model
    baseline_ladder.csv        machine-readable, for the LaTeX table
And under --root/results/malayalam/:
    baseline-ladder.md         both corpora side by side
"""
...
SEED = 20260828   # MUST match low_baselines.py
...
def run_corpus(name, corpus_dir, a):
    ...
    w(f"{k}-fold stratified CV, macro-F1, chance = {chance:.3f}")
    w("")

    results, preds = [], {}
    seeds = [SEED + i for i in range(a.repeats)]
    w(f"repeats: {a.repeats} seed(s) {seeds} — mean +/- std ACROSS seeds")
    w("  (seed-to-seed spread is larger than many of the gaps below; a")
    w("   single-seed number is not stable enough to report on its own)")
    w("")
    ...
    for label, model, why in build_ladder():
        ...
        if model is None:
            ...
            failed = False
            for sd_i in seeds:
                cvi = StratifiedKFold(k, shuffle=True, random_state=sd_i)
                try:
                    p = cross_val_predict(model, X, y, cv=cvi)
            ...
            if failed:
                continue
            f1, acc, sd = float(np.mean(f1s)), float(np.mean(accs)), float(np.std(f1s))
            p = last
        preds[label] = p
    ...
    if not ranked:
        ...
        with open(os.path.join(corpus_dir, "corpus", "prelim",
                               "baseline_ladder.txt"), "w", encoding="utf-8") as f:
            f.write("\n".join(out) + "\n")
        return None
    best = ranked[0]
    ...
    cpath = os.path.join(prelim, f"confusion_{tag}.png")
    confusion_png(y, preds[best[0]], labels, cpath, f"{name} — {best[0]}")
    w(f"confusion matrix: {cpath}")

    with open(os.path.join(prelim, "baseline_ladder.txt"), "w", encoding="utf-8") as f:
        f.write("\n".join(out) + "\n")
    with open(os.path.join(prelim, "baseline_ladder.csv"), "w",
              encoding="utf-8", newline="") as f:
        wr = csv.writer(f)
        wr.writerow(["model", "macro_f1", "accuracy", "fold_std", "hypothesis"])
        for r in results:
            wr.writerow([r[0], f"{r[1]:.4f}", f"{r[2]:.4f}", f"{r[3]:.4f}", r[4]])
    print(f"\nwritten: {os.path.join(prelim, 'baseline_ladder.txt')}")
...
def main(a):
    ...
    p = os.path.join(out_dir, "baseline-ladder.md")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--repeats", type=int, default=3,
                    help="how many seeds to average over (1 = fast, 3 = reportable)")
