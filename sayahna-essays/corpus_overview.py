...
def lengths(df, path, log=True):
    ...
    n = len(d)
    fig, ax = plt.subplots(figsize=(min(46, max(18, n * 0.21)), 11.5))
    fig.patch.set_facecolor("white")

    big = n > 200                     # essay corpora run to ~1000 works
    x = np.arange(n)
    bars = ax.bar(x, d.word_count, color=[colour(t) for t in d.wtype],
                  edgecolor="white" if not big else "none",
                  linewidth=0.5 if not big else 0.0,
                  width=0.82 if not big else 1.0, zorder=3)
    ...
    if big:
        ax.set_xticks([])             # 1000 labels would be an unreadable smear
    else:
        ax.set_xticks(x)
    ...
    ax.set_xlabel("Work" + ("  (individual labels omitted — see corpus_summary.csv)"
                          if n > 200 else ""), fontsize=12, labelpad=10)
...
def length_distribution(df, path):
    """Box plot of work length per author — readable at any corpus size."""
    order = (df.groupby("author").word_count.median()
               .sort_values(ascending=False).index.tolist())
    data = [df.loc[df.author == a, "word_count"].values for a in order]
    fig, ax = plt.subplots(figsize=(max(11, len(order) * 0.95), 9))
    fig.patch.set_facecolor("white")

    bp = ax.boxplot(data, vert=True, patch_artist=True, widths=0.62,
                    showfliers=False, medianprops=dict(color="#222222", lw=2))
    cmap = plt.get_cmap("tab20")
    for i, box in enumerate(bp["boxes"]):
        box.set(facecolor=cmap(i % 20), alpha=0.75, edgecolor="#555555", lw=1.0)
    ...
    for i, vals in enumerate(data, start=1):
        ...
        ax.scatter(rng.normal(i, 0.055, len(vals)), vals, s=9,
                   color="#222222", alpha=0.34, zorder=4, linewidths=0)

    ax.set_xticks(range(1, len(order) + 1))
    ax.set_xticklabels([textwrap.fill(a, 16) for a in order],
    ...
    ax.set_title("Distribution of work length per author\n"
                 "box = quartiles, line = median, dots = individual works",
                 fontsize=15, fontweight="bold", pad=16)
    for i, a in enumerate(order, start=1):
        ax.annotate(f"n={int((df.author == a).sum())}", (i, ax.get_ylim()[1]),
...
def main():
    ...
    length_distribution(df, os.path.join(PRELIM, "length_distribution.png"))
